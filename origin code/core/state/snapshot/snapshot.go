...
// Snapshot表示Snapshot存储层支持的功能。
type Snapshot interface {
	// Root returns the root hash for which this snapshot was made.
	//该 Sanpshot的 root Hash
	Root() common.Hash

	// Account directly retrieves the account associated with a particular hash in
	// the snapshot slim data format.
	//  通过 account hash  检索 account
	//如果不存在则返回一个 空账户
	Account(hash common.Hash) (*Account, error)

	// AccountRLP directly retrieves the account RLP associated with a particular
	// hash in the snapshot slim data format.
	//AccountRLP以snapshot slim数据格式直接检索与特定哈希关联的account RLP。
	//如果不存在则返回一个空byte
	AccountRLP(hash common.Hash) ([]byte, error)

	// Storage directly retrieves the storage data associated with a particular hash,
	// within a particular account.
	//直接检索特定帐户中与特定哈希关联的存储数据。
...
type snapshot interface {
	...
	//Journal将整个diff层次结构 以一个单独的日志条目中 提交到 disk
...
//
//SnapshotTree是以太坊状态快照树。它由一个由键值存储支持的持久性基础层组成，在这个基础层的顶部有任意多个内存差异层。
// 内存差异可以形成一个有分支的树，但是磁盘层是单一的，对所有人来说都是公共的。如果reorg比磁盘层更深，则需要删除所有内容。
...
type Tree struct {
	...
	cache  int                      // Megabytes permitted to use for read caches  允许用于读缓存的兆字节

	layers map[common.Hash]snapshot // Collection of all known layers  snapshot 的Headroot
	lock   sync.RWMutex
}
...
// background thread.
//尝试从持久键值存储（带有journal中的多个memory layers ）中加载一个已经存在的快照，以确保快照的头部与预期匹配。
//如果快照丢失或不一致，则会删除整个快照，并将根据后台线程中键值存储区中的尝试从头开始重建快照。
...
//如果可以将快照链接到现有的旧父对象，更新会将新快照添加到树中。 不允许插入磁盘层（所有磁盘的起源）。
