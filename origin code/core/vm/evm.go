...
// run runs the given contract and
// takes care of running precompiles with a fallback to the byte code interpreter.
//注意执行 precompiles 通过字节码解释器的fallback ？
/*
input: 在创建合约时该值为空
*/
...
func (evm *EVM) create(caller ContractRef, codeAndHash *codeAndHash, gas uint64, value *big.Int, address common.Address) ([]byte, common.Address, uint64, error) {
	...
	*/
	...
	caller:调用者
	address:合约地址
	value:调用者在创建合约的同时向合约账户转账value的ether
	gas:当前gas剩余量
	*/
	...
	c.CodeAddr = addr
	*/
	// 这个codeAndHash中没有codeHash 只有Code
	contract.SetCodeOptionalHash(&address, codeAndHash) //设置合约代码  codeAndHash中有合约源码
	...
	*/
	...
	/*9.如果一切都顺利，则计算存储代码所需要消耗的gas， 每字节200Gas*/
...
// Create creates a new contract using code as deployment code.
/* code = message.data() =  tx.txdata.payload()*/
